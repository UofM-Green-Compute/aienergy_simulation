...
num_of_pdf = np.array([1, 5, 10, 15])
#pages = np.array([1, 5, 10, 20, 30, 40, 50])
median_energy = []


for n_pdf in num_of_pdf:
    ...
    median_energy.append(np.median(energies))
...
with plt.rc_context({
    ...
    # Annotate each point
    for x, y in zip(num_of_pdf, median_energy):

        input_tokens = int(x * WORDS_PER_PAGE * TOKENS_PER_WORD * n_pages)
        output_tokens = int(x * SUMMARY_WORDS_PER_PAGE * TOKENS_PER_WORD * n_pages)
    ...
    ax.set_title("Energy required to summarise multiple PDFs of fixed page length")
    ...
    plt.savefig(
        "manuscript_figures/updated_figures/document_summary_scaling.pdf",
        bbox_inches="tight"
    )
